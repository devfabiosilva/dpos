...
#define F_SIGN_BUF_SZ (size_t)(sizeof(crypto_generichash_state)+64+32+sizeof(ge_p3)+sizeof(ge_p2))
...
int IRAM_ATTR f_nano_p2pow_to_JSON(char *buffer, size_t *olen, size_t buffer_sz, F_BLOCK_TRANSFER *block)
{
...
*/
int IRAM_ATTR f_is_valid_nano_seed_encrypted(void *stream, size_t stream_len, int read_from)
{
   ...
   const unsigned char *private_key)
{
   ...
   (output_type&F_SIGNATURE_RAW)?(memcpy(signature, tmp, 64)):(fhex2strv2((char *)signature, tmp, 64, 1));
...
int f_generate_token(F_TOKEN signature, void *data, size_t data_sz, const char *password)
{
   ...
   esp_fill_random(((uint8_t *)signature)+(sizeof(F_TOKEN)>>2), sizeof(F_TOKEN)>>2);
...
int nano_account_or_pk_string_to_pk_util(uint8_t *buffer, int *is_xrb_prefix, const unsigned char *str, size_t str_len)
{
   ...
   *is_xrb_prefix=0;
   ...
   if (((err=is_nano_prefix((const char *)str, NANO_PREFIX)))?(err<<=1):(err=is_nano_prefix((const char *)str, XRB_PREFIX))) {
      *is_xrb_prefix=err&1;
